...
class Typer:
    ...
    def __init__(
        self, accuracy=0.9, correction_chance=0.6, typing_delay=(0.04, 0.11), distance=1
    ):
    ...
    def send(self, element: WebElement, text: str):
        isError = False
        true = ""
        for a in text:
    ...
    def sendTextOneByOne(self, element: WebElement, text: str, send="all"):
    ...
    def getTuple(self, char):
        k = [
            ...
            for index, row in enumerate(self.qwertyKeyboardArray)
            if char in row
        ]
        ...
        if len(k) == 0:
            k = [
                (index, row.index(char))
                for index, row in enumerate(self.qwertyShiftedKeyboardArray)
                if char in row
            ]
